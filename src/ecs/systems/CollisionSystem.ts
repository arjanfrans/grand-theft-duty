...
export class CollisionSystem implements SystemInterface {
    ...
    private getEntities(): Entity[] {
        const entities = this.em.getEntitiesWithTypes(CollisionSystem.REQUIRED_COMPONENTS)
    ...
    private addBody(entity: Entity) {
        if (!this.bodies.has(entity)) {
            const dimensionComponent = entity.getComponent<DimensionComponent>(DimensionComponent.TYPE);
    ...
    update(delta: number): void {
        for (const entity of this.getEntities()) {
            ...
            const soldier = entity.getComponent<SoldierComponent>(SoldierComponent.TYPE);
            ...
            if (!alive.isDead) {
                ...
                CollisionChecker.wallCollision(this.map, position, movement, body, (response) => {
                    position.position.x -= response.overlapV.x;
                    position.position.y -= response.overlapV.y;
                });

                CollisionChecker.floorCollision(this.map, position, movement, body, delta, (block) => {
                    if (block.type === "water") {
                        movement.fall();
                        ...
                        position.position.z = block.position.z + block.depth;
